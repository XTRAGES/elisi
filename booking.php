...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $current_time = date('Y-m-d\TH:i', time());

    if ($checkin < $current_time) {
        echo "You cannot book a room in the past. Please choose a future date.";
    } else {

        $conn = new mysqli("localhost", "root", "", "elisi");

        if ($conn->connect_error) {
            die("Connection failed: " . $conn->connect_error);
        }

        $stmt = $conn->prepare("INSERT INTO bookings (firstlastname, email, checkin, checkout, adults, kids, rooms, specialrequests) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssss", $firstlastname, $email, $checkin, $checkout, $adults, $kids, $rooms, $specialrequests);
        $stmt->execute();
        $stmt->close();
        $conn->close();
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                <div class="form-floating date" id="date3" data-target-input="nearest">
                                    <input type="datetime-local" class="form-control" id="checkin" name="checkin" placeholder="Check In" data-target="#date3" required min="<?php echo date('Y-m-d\TH:i'); ?>" />
                                    <label for="checkin">Check In</label>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-floating date" id="date4" data-target-input="nearest">
                                    <input type="datetime-local" class="form-control" id="checkout" name="checkout" placeholder="Check Out" data-target="#date4" required min="<?php echo date('Y-m-d\TH:i'); ?>" />
                            ...
                            <div class="col-md-6">
                                <div class="form-floating">
                                    ...
                                    <label for="select1">Adults</label>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-floating">
                                    <select class="form-select" id="select2" name="kids">
                                    ...
                                    <label for="select2">Kids</label>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-floating">
                                    <select class="form-select" id="select3" name="rooms" required>
                                        <option value="1">1</option>
                                        <option value="2">2</option>
                                        <option value="3">3</option>
                                        <option value="4">4</option>
